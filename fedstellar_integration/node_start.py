...
from fedstellar.learning.pytorch.syscall.models.autoencoder import (
...
def main():
    ...
    additional_node_status = config.participant["mobility_args"]["additional_node"][
        "status"
    ]
    additional_node_round = config.participant["mobility_args"]["additional_node"][
        "round_start"
    ]
    ...
    target_changed_label = config.participant["adversarial_args"][
    ...
    if dataset == "MNIST":
        ...
        dataset = SYSCALLDataset(
            sub_id=idx, number_sub=n_nodes, root_dir=f"{sys.path[0]}/data", iid=iid
        )
        if model_name == "MLP":
        ...
        dataset = MalwaresDataset(sub_id=idx, number_sub=n_nodes, iid=iid)
        if model_name == "MLP":
    ...
    config.participant["REPORT_FREQUENCY"] = (
        (n_nodes * 0.4) + random.randint(-5, 5)
        if n_nodes > 10
        else 10 + random.randint(-5, 5)
    )
    ...
    if config.participant["device_args"]["start"]:
        time.sleep(
            config.participant["GRACE_TIME_START_FEDERATION"]
        )  # Wait for the grace time to start the federation (default is 20 seconds)

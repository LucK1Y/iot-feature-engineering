...
class DataSource(enum.Enum):
    ...
    SystemCalls = {}
    Entropy = {"file_path", "action", "entropy"}

    @classmethod
    def from_columns(cls, columns):
        cols_set = set(columns)

        if "index" in cols_set:
            cols_set.remove("index")
    ...
    @classmethod
    def from_single_column(cls, column: str):
        for data_type in cls:
            if column in data_type.value:
                return data_type
        raise ValueError(f"Unknown data type for column: {column}")

    @classmethod
    def from_string_name(cls, data_source: str):
        match data_source:
            case "SYS_data":
                return cls.SystemCalls
            case "KERN_data":
                return cls.KernelEvents
            case "FLS_data":
                return cls.FileSystem
            case "RES_data":
                return cls.ResourceUsageEvents
            case "block_data":
                return cls.BlockInputOutputEvents
            case "network_data":
                return cls.NetRecords
            case "entropy_data":
                return cls.Entropy
        raise ValueError(f"Unknown data type for string name: {data_source}")
